...
from api.schemas.audio import CreateAudioRequest, GetAudioRequest, ShowAudio, UpdateAudioRequest
from api.schemas.audio import UpdateAudioResponse, DeleteAudioRequest, DeleteAudioResponse


async def _create_audio(body: CreateAudioRequest, 
                        session: AsyncSession) -> ShowAudio:
    with session.begin():
        ...
        return ShowAudio(
            ...
            audio = new_audio.audio
        )

async def _get_audio(body: GetAudioRequest,
                     session: AsyncSession) -> ShowAudio:
    with session.begin():
        audio_dal = AudioDAL(session)
...
async def _update_audio(audio_id: UUID, 
                        body: UpdateAudioRequest,
                        session: AsyncSession) -> UpdateAudioResponse:
    with session.begin():
        audio_dal = AudioDAL(session)
...
async def _delete_audio(body: DeleteAudioRequest, 
                        session: AsyncSession) -> DeleteAudioResponse:
    with session.begin():
        audio_dal = AudioDAL(session)
